...
- [ ] DB 保存用の変換関数 (`convertToDbNutritionFormat` 等) が `nutrition-type-utils.ts` に実装または確認されている
- [ ] `MealService` のデータ取得メソッドで `convertToStandardizedNutrition` が適用されている
- [ ] `MealService` のデータ保存/更新メソッドで `convertToDbNutritionFormat` (または類似関数) が適用されている
- [ ] `RecipeService` の栄養データ関連メソッドが同様に更新されている
- [ ] Supabase モックデータが DB 構造とメモリ内構造の違いを反映している
- [ ] `MealService` と `RecipeService` の関連テストが正常に実行され、全てのアサーションが成功する
- [ ] コンソールに型エラーや変換関連のエラーが出力されていない
